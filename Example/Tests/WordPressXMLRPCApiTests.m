...
- (void)testGuessXMLRPCURLForSiteForFailedPluginRedirects {
    NSString *originalURL = @"http://mywordpresssite.com/xmlrpc.php";
    NSString *redirectedURL = @"https://mywordpresssite.com/xmlrpc.php";
    ...
    [OHHTTPStubs stubRequestsPassingTest:^BOOL(NSURLRequest *request) {
        ...
        NSURL *mockDataURL = [[NSBundle bundleForClass:[self class]] URLForResource:@"plugin_redirect" withExtension:@"html"];
        NSData *mockData = [NSData dataWithContentsOfURL:mockDataURL];
        return [[OHHTTPStubsResponse responseWithData:mockData statusCode:200 headers:nil]
                responseTime:OHHTTPStubsDownloadSpeedWifi];
    }];

    XCTestExpectation *expectation = [self expectationWithDescription:@"Call should fail"];
    [WordPressXMLRPCApi guessXMLRPCURLForSite:originalURL success:^(NSURL *xmlrpcURL) {
        XCTFail(@"Call to valid site should not enter success block.");
    } failure:^(NSError *error) {
        [expectation fulfill];
        XCTAssertTrue(error.domain == WordPressXMLRPCApiErrorDomain, @"Expected to get an WordPressXMLRPCApiErrorDomain error");
        XCTAssertTrue(error.code == WordPressXMLRPCApiMobilePluginRedirectedError, @"Expected to get an WordPressXMLRPCApiMobilePluginRedirectedError error");
    }];
    [self waitForExpectationsWithTimeout:5 handler:nil];
}
